...
write_gambenv_config() {
    write() {
        echo "$@" >> ${GB_GAMBIT_BUILD_CONFIG_FILE}
    }

    log "Writing gambenv configuration to ${GB_GAMBIT_BUILD_CONFIG_FILE}..."
    touch "${GB_GAMBIT_BUILD_CONFIG_FILE}"
    write ";; generated by gambenv ${GB_VERSION}"
    write ""
    write "((gambenv-version . \"${GB_VERSION}\")"
    write " (gambenv-gambit-version . \"${GB_GAMBIT_VERSION_ALIAS}\")"
